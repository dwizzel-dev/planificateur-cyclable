...
@auth:	Dwizzel
@date:	00-00-0000
@info:	default question/items/item-1/ view
	...
	<div class="row-fluid ">
		<div class="span12">
			<ul class="nav nav-pills">
				...
				<li><?php echo '<a href="#bulles" onclick="scrollToElement(\'#bulles\')">'._T('bulle aide').'</a>'; ?></li>
				<li><?php echo '<a href="#reponses" onclick="scrollToElement(\'#reponses\')">'._T('reponses').'</a>'; ?></li>
			</ul>
			<form class="form-vertical" id="form-item">
				...
				<div class="control-group">
					...
					<select name="grille_id" id="modify-input-grille_id">
					<option value="0">--</option>
					<?php
					foreach($arrOutput['content']['grille-dropdown'] as $k=>$v){
						echo '<option value="'.$v['id'].'" ';
						if($v['id'] == $arrOutput['content']['item']['details']['grille_id']){
							echo ' selected ';
							}
						echo '>'.$v['text'].'</option>'.EOL;
						}
				...
				<div class="control-group" >
					<a name="bulles" id="bulles"></a>
					<h3><?php echo _T('bulle aide'); ?><a onclick="scrollToElement('#page-top');" style="margin-left:20px;opacity:0.3;" href="#page-top"><img src="<?php echo PATH_IMAGE; ?>glyphicons/glyphicons_218_circle_arrow_top.png"></a></h3>
				</div>
				<div class="input-append">
					<select id="select-input-bulle-id" name="bulle_id">
					<option value="0">--</option>
					<?php
					foreach($arrOutput['content']['bulle-dropdown'] as $k=>$v){
						echo '<option ';
						if($arrOutput['content']['item']['details']['bulle_id'] == $v['id']){
							echo ' selected ';
							}
						echo 'value="'.$v['id'].'">#'.number_pad($v['id'],3).': '.$v['name'].'</option>'.EOL;
						}
					?>
					</select>
				</div>
				<div class="control-group" >
					<a name="reponses" id="reponses"></a>
					<h3><?php echo _T('reponses'); ?><a onclick="scrollToElement('#page-top');" style="margin-left:20px;opacity:0.3;" href="#page-top"><img src="<?php echo PATH_IMAGE; ?>glyphicons/glyphicons_218_circle_arrow_top.png"></a></h3>
				</div>
				<div class="input-append">
					<select id="select-input-items-id">
					<option value="0">--</option>
					<?php
					foreach($arrOutput['content']['reponse-dropdown'] as $k=>$v){
						echo '<option value="'.$v['id'].'">#'.number_pad($v['id'],3).': '.$v['name'].'</option>'.EOL;
						}
					?>
					</select>
					<button class="btn" id="buttinsert-items"><?php echo _T('insert reponse'); ?></button>
				</div>
				<p><small class="muted"><?php echo _T('select a reponse from the dropdown above then press the insert button.'); ?></small></p>
				<div id="div-items-main" style="margin-bottom:40px;margin-top:20px;width:100%;float:left;"  class="control-group">
					<?php
					$strItemsValues = '';
					if(is_array($arrOutput['content']['item']['reponses'])){
						foreach($arrOutput['content']['item']['reponses'] as $k=>$v){
							$strItemsValues .= $v['id'].',';
							$strLink = $oGlob->getArray('links','reponse-items').'item-'.$v['id'].'/';
	...
	if($('#div-items-main') && $('#select-input-items-id')){
		...
		if(id != 0){
			...
			if($.inArray(id, arrIds) == -1){
				...
				var code = '<div id="div-main-items-' + id + '" class="pull-left img-polaroid"  style="clear:both;position:relative;float:left;min-width:400px;max-width:400px;margin-bottom:3px;padding-right:25px;">' + text + '<div style="position:absolute;top:5px;right:5px;"><a onclick="removeItem(' + id + ')"><i class="icon-remove"></i></a></div></div>';
	...
	$.ajax({
		...
		success:function(data){
			...
			if(typeof(obj.msgerrors) !== "undefined" && obj.msgerrors){
				...
				window.location.href = '<?php echo $oGlob->getArray('links','question-listing').'page-'.$arrOutput['content']['question-listing-last-page-number'].'/'; ?>';
				}
			},
		error:function(){
			showButtLoadingTxt('#buttdelete', '<?php echo _T('delete'); ?>');
...
function saveDetails(){
	...
	$.ajax({
		type: 'POST',
		url: '<?php echo PATH_SERVICE; ?>',
		data: {section:'question',service:'set-item-infos',data:JSON.stringify($('#form-item').serializeArray())},
		success:function(data){
			//parse data
			//alert(data);
			eval('var obj = ' + data + ';');
...
function resetForm(){
	...
	$('#box-alert').hide();
	$('#box-alert-msg').text();
	var arr = [['name', 'val'], ['language_id', 'val'], ['grille_id', 'val'],];
	for(var o in arr){
