...
class MenuClient {
	...
	public function getMenuClientListing(){
		$query = 'SELECT '.DB_PREFIX.'links.path AS "path", '.DB_PREFIX.'menu_group.name AS "menu_group", '.DB_PREFIX.'menu.parent_id AS "parent_id", '.DB_PREFIX.'menu.position AS "position", '.DB_PREFIX.'menu.access AS "access", '.DB_PREFIX.'menu.id AS "id", '.DB_PREFIX.'menu.name AS "name", '.DB_PREFIX.'menu.status AS "status", '.DB_PREFIX.'menu.anchor AS "anchor", '.DB_PREFIX.'menu.target AS "target", '.DB_PREFIX.'languages.prefix AS "language" FROM '.DB_PREFIX.'menu LEFT JOIN '.DB_PREFIX.'menu_group ON '.DB_PREFIX.'menu_group.id = '.DB_PREFIX.'menu.menu_group_id LEFT JOIN '.DB_PREFIX.'languages ON '.DB_PREFIX.'languages.id = '.DB_PREFIX.'menu_group.language_id LEFT JOIN '.DB_PREFIX.'links ON '.DB_PREFIX.'links.id = '.DB_PREFIX.'menu.link_id WHERE '.DB_PREFIX.'menu.link_id = 0 AND '.DB_PREFIX.'menu.menu_type = "0" ORDER BY '.DB_PREFIX.'languages.prefix, '.DB_PREFIX.'menu.name;';
		$rs = $this->reg->get('db')->query($query);
		if($rs->num_rows){
			return $rs->rows;	
			}
	...
	public function setMenuClientInfos($arr){
		...
		if(isset($arr) && is_array($arr) && count($arr)){
			foreach($arr as $k=>$v){
				...
				}else if($v['name'] == 'anchor'){
					$arrValues['anchor'] = $v['value'];
				}else if($v['name'] == 'target'){
					$arrValues['target'] = $v['value'];	
				}else if($v['name'] == 'name'){
					if($v['value'] == ''){
						array_push($this->arrFormErrors, $v['name']);
	...
	public function getMenuClientInfos($id){	
		$query = 'SELECT '.DB_PREFIX.'menu.status AS "status", '.DB_PREFIX.'menu.id AS "id", '.DB_PREFIX.'menu.name AS "name", '.DB_PREFIX.'menu.anchor AS "anchor", '.DB_PREFIX.'menu.target AS "target", '.DB_PREFIX.'menu.position AS "position", '.DB_PREFIX.'menu.description AS "description", '.DB_PREFIX.'menu.menu_group_id AS "menu_group_id", '.DB_PREFIX.'menu.link_id AS "link_id", '.DB_PREFIX.'menu.parent_id AS "parent_id", '.DB_PREFIX.'menu.menu_type AS "menu_type" FROM '.DB_PREFIX.'menu WHERE '.DB_PREFIX.'menu.id = "'.$id.'" LIMIT 0,1;';
		$rs = $this->reg->get('db')->query($query);
		if($rs->num_rows){
			return $rs->rows[0];	
			}
