...
@auth:	Dwizzel
@date:	02-11-2013
@info:	modified for moxiecut russian version

- doit nmodifier les nom de fichier sur le upload
- creer different thumbnails avec .thumbnails
- creer les different format selon le repertoire ou oin est comme le catalogue par exemple qui a besoin de mini-thumbs-listing-photo-zoom
- hiiden les repertoire avec '.nomderepertoire'

*/

// DWIZZEL::base required
...
// DWIZZEL::base directory
//$workdir = DIR.$uploaddir;
$workdir = MOXIECUT_DIR.$uploaddir;
...
function GetData($path,$ext=false,$filter=false){
    ...
    foreach($files AS $file){
        ...
        $bInsert = true;
		if($val == 'dir'){
			//on check si premiere char .nomrepertoire
			if(substr($file,0,1) == '.'){
				$bInsert = false;
				}
