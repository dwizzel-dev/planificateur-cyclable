...
@auth:	Dwizzel
@date:	00-00-0000
@info:	bulle controller listing and items
...
if($section == 'listing'){
	...
	if(preg_match('/page-([0-9]+)\//',$oGlob->get('args_page'),$arrPregRes)){
		...
		$oReg->get('sess')->put('bulle-listing-last-page-number', $arrOutput['page']['start']);
		}
	//	
	if(preg_match('/language-([0-9]+)\//',$oGlob->get('args_page'),$arrPregRes)){
		$arrOutput['filter']['language-id'] = intVal($arrPregRes[1]);
		$oReg->get('sess')->put('bulle-listing-filter-language-id', $arrOutput['filter']['language-id']);
	}else if($oReg->get('sess')->get('bulle-listing-filter-language-id')){
		$arrOutput['filter']['language-id'] = $oReg->get('sess')->get('bulle-listing-filter-language-id');
		}	
	//	
	if(preg_match('/display-([0-9]+)\//',$oGlob->get('args_page'),$arrPregRes)){
		$arrOutput['filter']['display-id'] = intVal($arrPregRes[1]);
		$oReg->get('sess')->put('bulle-listing-filter-display-id', $arrOutput['filter']['display-id']);
	}else if($oReg->get('sess')->get('bulle-listing-filter-display-id') || $oReg->get('sess')->get('bulle-listing-filter-display-id') === 0){
		$arrOutput['filter']['display-id'] = $oReg->get('sess')->get('bulle-listing-filter-display-id');
		}
	//
	if(preg_match('/sortby-([a-z_]+)\//',$oGlob->get('args_page'),$arrPregRes)){
		$arrOutput['filter']['sortby-id'] = $arrPregRes[1];
		$oReg->get('sess')->put('bulle-listing-filter-sortby-id', $arrOutput['filter']['sortby-id']);
	}else if($oReg->get('sess')->get('bulle-listing-filter-sortby-id')){
		$arrOutput['filter']['sortby-id'] = $oReg->get('sess')->get('bulle-listing-filter-sortby-id');
		}	
	//
	if(preg_match('/sortdirection-([A-Z_]+)\//',$oGlob->get('args_page'),$arrPregRes)){
		$arrOutput['filter']['sortdirection-id'] = $arrPregRes[1];
		$oReg->get('sess')->put('bulle-listing-filter-sortdirection-id', $arrOutput['filter']['sortdirection-id']);
	}else if($oReg->get('sess')->get('bulle-listing-filter-sortdirection-id')){
		$arrOutput['filter']['sortdirection-id'] = $oReg->get('sess')->get('bulle-listing-filter-sortdirection-id');
		}	
	//
	if(preg_match('/searchitems-0\//',$oGlob->get('args_page'),$arrPregRes)){
		$arrOutput['filter']['searchitems-id'] = '';
		$oReg->get('sess')->put('bulle-listing-filter-searchitems-id', '');
	}else if(preg_match('/searchitems-(.+)\//',$oGlob->get('args_page'),$arrPregRes)){
		$arrOutput['filter']['searchitems-id'] = $arrPregRes[1];
		$oReg->get('sess')->put('bulle-listing-filter-searchitems-id', $arrOutput['filter']['searchitems-id']);
	}else if($oReg->get('sess')->get('bulle-listing-filter-searchitems-id')){
		$arrOutput['filter']['searchitems-id'] = $oReg->get('sess')->get('bulle-listing-filter-searchitems-id');
	}else{
		$arrOutput['filter']['searchitems-id'] = '';
		}
	...
	//title
	$arrOutput['content']['title'] = _T('bulle items listing');
	$arrOutput['meta']['title'] = $arrOutput['content']['title'];
	//la view
	$oGlob->set('page_view', 'bulle-listing');
	//le data
	...
	$arrOutput['content']['items']['rows'] = $oBulle->getBulleListing(($arrOutput['page']['start'] * $arrOutput['page']['limit']), $arrOutput['page']['limit'], $arrOutput['filter']);
	//total varie selon le searchitems-id, si vide les page au complet sinon ca depend du resultat
	$arrOutput['page']['total'] = $oBulle->getBulleListingCount($arrOutput['filter']);
	//widget pagination
	...
	$arrOutput['content']['bulle-listing-last-page-number'] = 0;
	if($oReg->get('sess')->get('bulle-listing-last-page-number')){
		$arrOutput['content']['bulle-listing-last-page-number'] = $oReg->get('sess')->get('bulle-listing-last-page-number');
		}
	//title
	$arrOutput['content']['title'] = _T('bulle item details');
	$arrOutput['meta']['title'] = $arrOutput['content']['title'];
	//la view
	$oGlob->set('page_view', 'bulle-item');
	//dropdown
	$arrOutput['content']['language-dropdown'] =  $oGlob->get('languages_for_dropbox');
	//le data
	require_once(DIR_CLASS.'bulle.php');
	...
	$arrOutput['content']['item']['details'] = $oBulle->getBulleInfos($arrOutput['content']['item-id']);
