...
@auth:	Dwizzel
@date:	00-00-0000
@info:	recommandation controller listing and items
...
if($section == 'listing'){
	...
	if(preg_match('/page-([0-9]+)\//',$oGlob->get('args_page'),$arrPregRes)){
		...
		$oReg->get('sess')->put('recommandation-listing-last-page-number', $arrOutput['page']['start']);
		}
	//	
	if(preg_match('/language-([0-9]+)\//',$oGlob->get('args_page'),$arrPregRes)){
		$arrOutput['filter']['language-id'] = intVal($arrPregRes[1]);
		$oReg->get('sess')->put('recommandation-listing-filter-language-id', $arrOutput['filter']['language-id']);
	}else if($oReg->get('sess')->get('recommandation-listing-filter-language-id')){
		$arrOutput['filter']['language-id'] = $oReg->get('sess')->get('recommandation-listing-filter-language-id');
		}	
	//	
	if(preg_match('/display-([0-9]+)\//',$oGlob->get('args_page'),$arrPregRes)){
		$arrOutput['filter']['display-id'] = intVal($arrPregRes[1]);
		$oReg->get('sess')->put('recommandation-listing-filter-display-id', $arrOutput['filter']['display-id']);
	}else if($oReg->get('sess')->get('recommandation-listing-filter-display-id') || $oReg->get('sess')->get('recommandation-listing-filter-display-id') === 0){
		$arrOutput['filter']['display-id'] = $oReg->get('sess')->get('recommandation-listing-filter-display-id');
		}
	//
	if(preg_match('/sortby-([a-z_]+)\//',$oGlob->get('args_page'),$arrPregRes)){
		$arrOutput['filter']['sortby-id'] = $arrPregRes[1];
		$oReg->get('sess')->put('recommandation-listing-filter-sortby-id', $arrOutput['filter']['sortby-id']);
	}else if($oReg->get('sess')->get('recommandation-listing-filter-sortby-id')){
		$arrOutput['filter']['sortby-id'] = $oReg->get('sess')->get('recommandation-listing-filter-sortby-id');
		}	
	//
	if(preg_match('/sortdirection-([A-Z_]+)\//',$oGlob->get('args_page'),$arrPregRes)){
		$arrOutput['filter']['sortdirection-id'] = $arrPregRes[1];
		$oReg->get('sess')->put('recommandation-listing-filter-sortdirection-id', $arrOutput['filter']['sortdirection-id']);
	}else if($oReg->get('sess')->get('recommandation-listing-filter-sortdirection-id')){
		$arrOutput['filter']['sortdirection-id'] = $oReg->get('sess')->get('recommandation-listing-filter-sortdirection-id');
		}	
	//
	if(preg_match('/searchitems-0\//',$oGlob->get('args_page'),$arrPregRes)){
		$arrOutput['filter']['searchitems-id'] = '';
		$oReg->get('sess')->put('recommandation-listing-filter-searchitems-id', '');
	}else if(preg_match('/searchitems-(.+)\//',$oGlob->get('args_page'),$arrPregRes)){
		$arrOutput['filter']['searchitems-id'] = $arrPregRes[1];
		$oReg->get('sess')->put('recommandation-listing-filter-searchitems-id', $arrOutput['filter']['searchitems-id']);
	}else if($oReg->get('sess')->get('recommandation-listing-filter-searchitems-id')){
		$arrOutput['filter']['searchitems-id'] = $oReg->get('sess')->get('recommandation-listing-filter-searchitems-id');
	}else{
		$arrOutput['filter']['searchitems-id'] = '';
		}
	...
	//title
	$arrOutput['content']['title'] = _T('recommandation items listing');
	$arrOutput['meta']['title'] = $arrOutput['content']['title'];
	//la view
	$oGlob->set('page_view', 'recommandation-listing');
	//le data
	...
	$arrOutput['content']['items']['rows'] = $oRecommandation->getRecommandationListing(($arrOutput['page']['start'] * $arrOutput['page']['limit']), $arrOutput['page']['limit'], $arrOutput['filter']);
	//total varie selon le searchitems-id, si vide les page au complet sinon ca depend du resultat
	$arrOutput['page']['total'] = $oRecommandation->getRecommandationListingCount($arrOutput['filter']);
	//widget pagination
	...
	$arrOutput['content']['recommandation-listing-last-page-number'] = 0;
	if($oReg->get('sess')->get('recommandation-listing-last-page-number')){
		$arrOutput['content']['recommandation-listing-last-page-number'] = $oReg->get('sess')->get('recommandation-listing-last-page-number');
		}
	//title
	$arrOutput['content']['title'] = _T('recommandation item details');
	$arrOutput['meta']['title'] = $arrOutput['content']['title'];
	//la view
	$oGlob->set('page_view', 'recommandation-item');
	//le data
	require_once(DIR_CLASS.'recommandation.php');
	$oRecommandation = new Recommandation($oReg);
	//data	
	$arrOutput['content']['item'] = array();
	$arrOutput['content']['item']['details'] = $oRecommandation->getRecommandationInfos($arrOutput['content']['item-id']);
	$arrOutput['content']['item']['h-title'] = $arrOutput['content']['item']['details']['name'];
	//conseils
	$arrOutput['content']['item']['conseils'] = $arrOutput['content']['item']['details']['conseils'];
	//dropdown
	$arrOutput['content']['language-dropdown'] =  $oGlob->get('languages_for_dropbox');
	//dropdown des choix de conseil a asscoieavec la recommandation
	require_once(DIR_CLASS.'utils.php');
	$oUtils = new Utils($oReg);
	$arrOutput['content']['conseil-dropdown'] = $oUtils->getConseil($arrOutput['content']['item']['details']['language_id']);
		
	
	
	//clean
	unset($oRecommandation);
