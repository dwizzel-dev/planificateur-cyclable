...
@auth:	Dwizzel
@date:	00-00-0000
@info:	default bulle items listing view
...
jQuery(document).ready(function($){
	...
	$('#filter-language').change(function(){
		window.location.href = '<?php echo $oGlob->getArray('links','bulle-listing')?>page-0/language-' + $(this).attr('value') + '/';
		});	
	$('#filter-display').change(function(){
		window.location.href = '<?php echo $oGlob->getArray('links','bulle-listing')?>page-0/display-' + $(this).attr('value') + '/';
		});	
	$('#filter-sortby').change(function(){
		window.location.href = '<?php echo $oGlob->getArray('links','bulle-listing')?>page-0/sortby-' + $(this).attr('value') + '/';
		});	
	$('#filter-sortdirection').change(function(){
		window.location.href = '<?php echo $oGlob->getArray('links','bulle-listing')?>page-0/sortdirection-' + $(this).attr('value') + '/';
		});		
	//delete items
	...
	$('#buttcreate').click(function(e){
		e.preventDefault();
		$('#modal-modify-items-title').text('<?php echo formatJavascript(_T('new bulle item')); ?>');
	...
	$('#buttsearchitems').click(function(e){
		...
		if($('#filter-searchitems').attr('value') != ''){
			window.location.href = '<?php echo $oGlob->getArray('links','bulle-listing')?>page-0/searchitems-' + $('#filter-searchitems').attr('value') + '/';
		}else{
			window.location.href = '<?php echo $oGlob->getArray('links','bulle-listing')?>page-0/searchitems-0/';
			}
		});	
	//clear serach
...
function setItemsInfos(){
	...
	$.ajax({
		...
		data: {section:'bulle',service:'new-item-infos',data:JSON.stringify($('#form-modify').serializeArray())},
...
function getItemInfos(strid){
	window.location.href = '<?php echo $oGlob->getArray('links','bulle-items')?>item-' + strid + '/';
	}
...
function deleteItem(){
	...
	$.ajax({
		type: 'POST',
		url: '<?php echo PATH_SERVICE; ?>',
		data: {section:'bulle',service:'delete-item-infos',data:JSON.stringify($('#form-process').serializeArray())},
		success:function(data){
			//parse data
			eval('var obj = ' + data + ';');
			if(typeof(obj.msgerrors) !== "undefined" && obj.msgerrors){
...
function disableItem(){
	...
	$.ajax({
		type: 'POST',
		url: '<?php echo PATH_SERVICE; ?>',
		data: {section:'bulle',service:'disable-item-infos',data:JSON.stringify($('#form-process').serializeArray())},
		success:function(data){
			//parse data
			eval('var obj = ' + data + ';');
			if(typeof(obj.msgerrors) !== "undefined" && obj.msgerrors){
...
function enableItem(){
	...
	$.ajax({
		type: 'POST',
		url: '<?php echo PATH_SERVICE; ?>',
		data: {section:'bulle',service:'enable-item-infos',data:JSON.stringify($('#form-process').serializeArray())},
		success:function(data){
			//parse data
			eval('var obj = ' + data + ';');
			if(typeof(obj.msgerrors) !== "undefined" && obj.msgerrors){
