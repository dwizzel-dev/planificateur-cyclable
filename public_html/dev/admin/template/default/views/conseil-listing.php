...
@auth:	Dwizzel
@date:	00-00-0000
@info:	default conseil items listing view
...
jQuery(document).ready(function($){
	...
	$('#filter-language').change(function(){
		window.location.href = '<?php echo $oGlob->getArray('links','conseil-listing')?>page-0/language-' + $(this).attr('value') + '/';
		});	
	$('#filter-display').change(function(){
		window.location.href = '<?php echo $oGlob->getArray('links','conseil-listing')?>page-0/display-' + $(this).attr('value') + '/';
		});	
	$('#filter-sortby').change(function(){
		window.location.href = '<?php echo $oGlob->getArray('links','conseil-listing')?>page-0/sortby-' + $(this).attr('value') + '/';
		});	
	$('#filter-sortdirection').change(function(){
		window.location.href = '<?php echo $oGlob->getArray('links','conseil-listing')?>page-0/sortdirection-' + $(this).attr('value') + '/';
		});		
	//delete items
	...
	$('#buttcreate').click(function(e){
		e.preventDefault();
		$('#modal-modify-items-title').text('<?php echo formatJavascript(_T('new conseil item')); ?>');
	...
	$('#buttsearchitems').click(function(e){
		...
		if($('#filter-searchitems').attr('value') != ''){
			window.location.href = '<?php echo $oGlob->getArray('links','conseil-listing')?>page-0/searchitems-' + $('#filter-searchitems').attr('value') + '/';
		}else{
			window.location.href = '<?php echo $oGlob->getArray('links','conseil-listing')?>page-0/searchitems-0/';
			}
		});	
	//clear serach
...
function setItemsInfos(){
	...
	$.ajax({
		...
		data: {section:'conseil',service:'new-item-infos',data:JSON.stringify($('#form-modify').serializeArray())},
...
function getItemInfos(strid){
	window.location.href = '<?php echo $oGlob->getArray('links','conseil-items')?>item-' + strid + '/';
	}
