...
@auth:	Dwizzel
@date:	00-00-0000
@info:	default content items listing product view
	...
	<div class="row-fluid thick-border-b" style="margin-bottom:20px;">
		<div class="span12 inline">
			<div class="pull-left">
				...
				<?php
				echo '<option value="">'._T('category filter').'</option>'.EOL;	
				foreach($arrOutput['content']['category-dropdown'] as $k=>$v){
					echo '<option value="'.$v['id'].'"';
					if(isset($arrOutput['content']['category-dropdown-selected']) && $arrOutput['content']['category-dropdown-selected'] == $v['id']){
						echo ' selected ';
						}
...
<div id="modal-modify-items" class="modal hide fade" tabindex="-1" role="dialog" aria-labelledby="" aria-hidden="true">
	...
	<div class="modal-body" id="modal-modify-items-body">
		...
		<form id="form-modify" method="post">
			...
			<select name="category_id" class="input-xlarge" id="modal-modify-input-category_id">
			<?php
			foreach($arrOutput['content']['category-dropdown'] as $k=>$v){
			...
			<label><?php echo _T('extra css class'); ?> :</label>
			<input type="text" name="css_class" id="modal-modify-input-css_class" class="w96">
		</form>
		</div>
	</div>
	<div class="modal-footer">
...
jQuery(document).ready(function($){
	...
	$('#filter-categorie').change(function(){
		window.location.href = '<?php echo $oGlob->getArray('links','content-listing')?>page-0/category-' + $(this).attr('value') + '/';
		});
	$('#filter-language').change(function(){
		window.location.href = '<?php echo $oGlob->getArray('links','content-listing')?>page-0/language-' + $(this).attr('value') + '/';
		});	
	$('#filter-display').change(function(){
		window.location.href = '<?php echo $oGlob->getArray('links','content-listing')?>page-0/display-' + $(this).attr('value') + '/';
		});	
	$('#filter-sortby').change(function(){
		window.location.href = '<?php echo $oGlob->getArray('links','content-listing')?>page-0/sortby-' + $(this).attr('value') + '/';
		});	
	//delete items
	...
	$('#buttcreate').click(function(e){
		e.preventDefault();
		$('#modal-modify-items-title').text('<?php echo formatJavascript(_T('new content item')); ?>');
	...
	$('#buttsearchitems').click(function(e){
		...
		if($('#filter-searchitems').attr('value') != ''){
			window.location.href = '<?php echo $oGlob->getArray('links','content-listing')?>page-0/searchitems-' + $('#filter-searchitems').attr('value') + '/';
		}else{
			window.location.href = '<?php echo $oGlob->getArray('links','content-listing')?>page-0/searchitems-0/';
			}
		});	
	//clear serach
...
function setItemsInfos(){
	...
	$.ajax({
		...
		data: {section:'content',service:'new-item-infos',data:JSON.stringify($('#form-modify').serializeArray())},
...
function resetForm(bText){
	...
	$('#modal-modify-items-body').scrollTo(0, 500);	
	var arr = [['status', 'checkbox'], ['css_class', 'val'], ['name', 'val'], ['category_id', 'val'], ['language_id', 'val'], ['meta_title', 'val'], ['meta_description', 'val'], ['meta_keywords', 'val'], ['content', 'val'], ];
	for(var o in arr){
		if(bText){
			if(arr[o][1] == 'text'){
...
function getItemInfos(strid){
	window.location.href = '<?php echo $oGlob->getArray('links','content-items')?>item-' + strid + '/';
	}
