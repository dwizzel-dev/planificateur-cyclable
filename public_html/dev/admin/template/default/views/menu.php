	...
	<div class="modal-body" id="modal-modify-items-body">
		<div id="modal-modify-items-content">
			...
			<form id="form-modify">
				...
				<label><?php echo _T('anchor'); ?> :</label>
				<input type="text" name="anchor" id="modal-modify-input-anchor" class="w20">
				<label><?php echo _T('target'); ?> :</label>
				<input type="text" name="target" id="modal-modify-input-target" class="w20">
...
function resetForm(bText){
	...
	var arr = [['status', 'checkbox'], ['name', 'val'], ['position', 'val'], ['description', 'text'], ['menu_group_id', 'val'], ['link_id', 'val'], ['parent_id', 'val'], ['menu_type', 'val'], ['anchor', 'text'], ['target', 'text']];
	for(var o in arr){
		if(bText){
			if(arr[o][1] == 'text'){
	...
	$.ajax({
		...
		success:function(data){
			...
			if(typeof(obj.msgerrors) !== "undefined" && obj.msgerrors){
				...
				$('#modal-modify-input-target').val(htmlspecialchars_decode(obj.target));
				$('#modal-modify-input-anchor').val(htmlspecialchars_decode(obj.anchor));
