...
function JAppz(){
	...
	this.showGrille = function(bTop, bBottom){
		...
		if(bTop){
			...
			str += '<h4>';
			str += '<p class="small">' + gData.keyvalue.type[this.arrCurrentGrilleInfos.type] + ' / ' + gData.keyvalue.milieu[this.arrCurrentGrilleInfos.milieu] + '</p>';
		...
		if(bBottom){
			...
			this.debug('showGrille::oGrille', oGrille);
			var strQuestion = '';
			strQuestion += '<ul id="listing-questions" class="link">';
			for(var o in oGrille.questions){
				strQuestion += '<li>';
				if(0){
					strQuestion += '<span class="left green"><small>&#9899;</small></span>';
				}else{
					strQuestion += '<span class="left red"><small>&#9899;</small></span>';
					}
			...
			$('#listing-questions > LI > A').click(function(e){
				...
				var oTmp = $(document).data('jappzclass');
				if(typeof(oTmp) == 'object'){
					oTmp.openQuestion(parseInt($(this).attr('question-id')));
					}
				});
	...
	//-----------------------------------------------------------------------------------------*
	this.openQuestion = function(id){
		this.debug('openQuestion(' + id + ')');
		
		
		if(this.arrCurrentQuestionsInfos !== false){
	...
	this.showParcours = function(bTop, bLeft, bRight){
		...
		if(bLeft){
			...
			/*
			//sans arbre de troncon-intersection
			var strGrille = '';
			for(var o in gParcours){
					...
					if(gParcours[o].grille != '' && gParcours[o].grille !== false){
						...
						for(var p in gParcours[o].grille){
							...
							if(gParcours[o].grille[p].status == 1){
								strGrille += '<span class="right green"><small>&#9899;</small></span>';
							}else{
								strGrille += '<span class="right red"><small>&#9899;</small></span>';
								}
							strGrille += '</li>';
							}
						strGrille += '</ul>';		
						...
						for(var p in arrDisplay){
							...
							if(typeof(arrKeepIntersection[arrDisplay[p].id]) == 'object'){
								for(var q in arrKeepIntersection[arrDisplay[p].id]){
									...
									//status
									if(arrKeepIntersection[arrDisplay[p].id][q].status == 1){
										strGrille += '<span class="right green"><small>&#9899;</small></span>';
									}else{
										strGrille += '<span class="right red"><small>&#9899;</small></span>';
										}
