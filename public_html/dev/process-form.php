...
if($sendType == 'inscription'){ //--------------------------------------------------------------------------------------
	...
	Header('Location: '.PATH_WEB_NORMAL.$oGlob->getArray('links', '6').$strLinkAppend.$anchorForm);	
	exit();
	...
	if($rs->num_rows){
		...
		setcookie('logued_in', '0');
		Header('Location: '.$oGlob->getArray('links', '6').'&err=101'.$anchorForm);
		exit();
		}	
