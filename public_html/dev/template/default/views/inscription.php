...
//-------------------------------------------------------
function chkPsw(){
	var s = $('#mot_de_passe_1').val();
	if(s != ''){
		if(/(?=.*\d)(?=.*[a-z])(?=.*[A-Z]).{8,32}/.test(s)){
			//
		}else{
			$('#modal-alert-title').text('<?php echo formatJavascript(_T('error')); ?>');
			$('#modal-alert-content').html('<?php echo formatJavascript(_T('password must contain')); ?>');
			$('#modal-alert-close').text('<?php echo formatJavascript(_T('close')); ?>');
			$('#modal-alert-link').text('');
